...
describe('triggers', () => {
  ...
  describe('listenToTriggers', () => {
    it('should set up event listeners to non-manual triggers', () => {
      const rendererSpy = {
        listen: jasmine.createSpy('listen')
      };
      rendererSpy.listen.and.callFake(() => {
        return () => {
        };
      });
      expect(() => {
        listenToTriggers(rendererSpy, null, 'mouseenter', null, null, null)();
      }).not.toThrow();
    });

    it('should set up event listeners to manual triggers', () => {
      const rendererSpy = {
        listen: jasmine.createSpy('listen')
      };
      rendererSpy.listen.and.callFake(() => {
      ...
      expect(() => {
        listenToTriggers(rendererSpy, null, 'manual', null, null, null)();
      }).not.toThrow();
    });

    it('should handle differing open and close triggers', () => {
      const rendererSpy = {
        listen: jasmine.createSpy('listen')
      };
      rendererSpy.listen.and.callFake(() => {
      ...
      expect(() => {
        listenToTriggers(rendererSpy, null, 'mouseenter:mouseleave', null, null, null)();
      }).not.toThrow();
    });
  });
});
