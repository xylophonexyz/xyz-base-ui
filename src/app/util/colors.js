...
function getAnalogousColors(color) {
  return tinycolor(color).analogous().map(function (t) {
...
function getMonochromaticColors(color) {
  return tinycolor(color).monochromatic().map(function (t) {
    return t.toHexString();
  });
}
...
function getSplitComplementColors(color) {
  return tinycolor(color).splitcomplement().map(function (t) {
    return t.toHexString();
  });
}
...
function getTriadColors(color) {
  return tinycolor(color).triad().map(function (t) {
    return t.toHexString();
  });
}
...
function getTetradColors(color) {
  return tinycolor(color).tetrad().map(function (t) {
    return t.toHexString();
  });
}
